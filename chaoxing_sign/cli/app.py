...
def show_and_select_tasks(tasks: list) -> list:
    ...
    for i, task in enumerate(active_tasks, 1):
        print(f"  [{i}] [{task.sign_type.label}] {task.name} - {task.course_name}")

    if ended_tasks:
        print(f"\n已结束签到 ({len(ended_tasks)} 个):")
        for task in ended_tasks:
            print(f"  [-] [{task.sign_type.label}] {task.name} - {task.course_name}")
...
def do_sign(client: ChaoxingClient, task, location_config: dict):
    """执行单个签到"""
    print(f"\n  >>> 正在签到: [{task.sign_type.label}] {task.name}")
